...
class OllamaProvider(ModelProvider):
    ...
    async def generate(
        ...
        try:
            ...
            raise ProviderError(
                f"Ollama HTTP error: {e.response.status_code}", provider="ollama"
            ) from e
        except httpx.NetworkError as e:
            logger.error("Ollama network error", error=str(e))
            raise ProviderError("Network connection to Ollama failed", provider="ollama") from e
        except Exception as e:
            logger.error("Ollama generation error", error=str(e))
            raise ProviderError(f"Ollama generation failed: {e}", provider="ollama") from e

    async def stream(
        self, messages: list[Message], model: str, temperature: float = 0.7, max_tokens: int = 2048
    ) -> AsyncIterator[str]:
    ...
    async def embed(self, text: str, model: str = "nomic-embed-text") -> list[float]:
        ...
        try:
            ...
            raise ProviderError(f"Failed to generate embeddings: {e}", provider="ollama") from e
